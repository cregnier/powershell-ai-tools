...
Get-ChildItem -Path $TemplatesDir -Filter '*.mcp.json' | ForEach-Object {
    ...
    $front += "agent: agent"
    # argument-hint and model: use values from content if present, otherwise sensible defaults
    $argHint = if ($content.PSObject.Properties.Name -contains 'argumentHint') { $content.argumentHint } elseif ($content.PSObject.Properties.Name -contains 'argument-hint') { $content.'argument-hint' } else { '' }
    $front += "argument-hint: $argHint"
    $modelVal = if ($content.PSObject.Properties.Name -contains 'model') { $content.model } else { 'gpt-4o-mini' }
    $front += "model: $modelVal"
    if ($content.tags) {
        $front += 'tools:'
