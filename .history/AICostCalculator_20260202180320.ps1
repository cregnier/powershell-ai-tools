...
if (-not $env:AI_MODE_OVERRIDE -and -not $GitHubUsageReportCsv) {
    Write-Host "\n" -NoNewline
    ...
    Write-Host "\nAuto-selecting Real Usage Analysis Mode (GitHub CSV provided)..." -ForegroundColor Yellow
    $operationalMode = "analysis"
    $FetchMode = $true
} else {
...
function Show-DualWindowDashboard {
    ...
    Write-Host (" " * (67 - "$([math]::Round($currentMonthTotal, 0)) requests".Length - (if ($currentMonthOverage -gt 0) { " ($([math]::Round($currentMonthOverage, 0)) overage = `$$($currentMonthCost.ToString('N2')))".Length } else { " (no overage)".Length }))) -NoNewline
    Write-Host "│" -ForegroundColor Magenta
    Write-Host "  └─────────────────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
    ...
    # Project to end of billing cycle based on historical avg
    ...
    Write-Host " Billing Cycle Forecast (based on avg $([math]::Round($avgDaily, 1))/day):" -ForegroundColor White -NoNewline
    Write-Host (" " * (67 - " Billing Cycle Forecast (based on avg $([math]::Round($avgDaily, 1))/day):".Length)) -NoNewline
    Write-Host "│" -ForegroundColor Cyan
    Write-Host "  │" -ForegroundColor Cyan -NoNewline
    ...
    if ($daysRemaining -le 5) {
        Write-Host "     • 📅 Billing cycle ends soon - prepare for next period" -ForegroundColor Cyan
    }
    Write-Host ""
}
