...
if ($FetchMode) {
    ...
    $dualCosts = Calculate-DualWindowCosts -dailyRows $dailyRows -cycleStartDay $CycleStartDay -basePlanRequests $DefaultBasePlanRequests -basePlanPrice $BasePlanPrice -overagePrice $OveragePricePerRequest
    
    Write-Host "  Billing Cycle ($($dualCosts.CycleStart.ToString('MMM dd')) - $($dualCosts.CycleEnd.ToString('MMM dd'))):" -ForegroundColor Yellow
    Write-Host "    • Cycle Total:        $([math]::Round($dualCosts.CycleTotal, 2)) requests" -ForegroundColor White
    Write-Host "    • Included (300):     $([math]::Round($dualCosts.CycleIncludedUsed, 2)) used" -ForegroundColor $(if ($dualCosts.CycleIncludedUsed -ge 300) { 'Red' } else { 'Green' })
    Write-Host "    • Beyond Quota:       $([math]::Round($dualCosts.CycleOverage, 2)) requests" -ForegroundColor $(if ($dualCosts.CycleOverage -gt 0) { 'Red' } else { 'Green' })
    Write-Host "    • Percent Used:       $($dualCosts.CyclePercentUsed.ToString('P2'))" -ForegroundColor $(if ($dualCosts.CyclePercentUsed -gt 1.0) { 'Red' } elseif ($dualCosts.CyclePercentUsed -gt 0.9) { 'Yellow' } else { 'Green' })
    Write-Host "" -ForegroundColor White
    ...
    foreach ($monthKey in ($dualCosts.MonthlyOverages.Keys | Sort-Object)) {
        ...
        if ($monthData.Overage -gt 0) {
            Write-Host "    • $monthKey`:          $([math]::Round($monthData.Overage, 2)) requests = `$$($monthData.OverageCost.ToString('N2'))" -ForegroundColor Red
        } else {
            Write-Host "    • $monthKey`:          No overage" -ForegroundColor Green
        }
    }
    Write-Host "" -ForegroundColor White
    
    Write-Host "  Total Costs:" -ForegroundColor Yellow
    Write-Host "    • Base Plan:          `$$($BasePlanPrice.ToString('N2'))" -ForegroundColor White
    Write-Host "    • Overage Charges:    `$$($dualCosts.TotalOverageCost.ToString('N2'))" -ForegroundColor $(if ($dualCosts.TotalOverageCost -gt 0) { 'Red' } else { 'Green' })
    Write-Host "    • TOTAL COST:         `$$($dualCosts.TotalCost.ToString('N2'))" -ForegroundColor $(if ($dualCosts.TotalCost -gt 20) { 'Red' } else { 'Green' })
    Write-Host "" -ForegroundColor White
