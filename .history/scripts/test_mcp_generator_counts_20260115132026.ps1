...
foreach ($n in $Counts) {
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -File ..\mcp_generator.ps1 -Name $name -Description "Count test $n" -Author "Tester" -Tags "count,test" -BodyFile $bodyFile -OutDir $OutDir -AddSystemPrompt -SystemPromptFile $SystemPromptFile -AsMarkdown -Force
