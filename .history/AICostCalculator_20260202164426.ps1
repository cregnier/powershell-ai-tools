...
# If running in fetch mode, auto-populate interactive inputs from the provided daily CSV (local or GitHub)
if ($FetchMode) {
    Write-Host 'Running in FetchMode: auto-populating inputs from daily usage data.' -ForegroundColor Cyan
    $fetchedCsv = $null
    if ($GitHubOwner -and $GitHubRepo -and $GitHubPath) {
        ...
        if ($content) {
            ...
            Write-Host 'Failed to fetch CSV from GitHub in FetchMode.' -ForegroundColor Yellow
        }
    }
    if (-not $fetchedCsv -and $DailyUsageCsv -and (Test-Path $DailyUsageCsv)) { $fetchedCsv = $DailyUsageCsv }
    if (-not $fetchedCsv) { Write-Host 'FetchMode requires either GitHubOwner/Repo/Path or a local -DailyUsageCsv.' -ForegroundColor Red; exit 1 }

    $dailyRows = Import-DailyUsage $fetchedCsv
    if (-not $dailyRows) { Write-Host 'No daily rows found in fetched CSV.' -ForegroundColor Red; exit 1 }

    # Auto-set interactive variables
    $givenDate = Get-Date
    # Start of workday default
    $startDt = Parse-HHMM '09:00'
    # Current time set to now
    $now = Get-Date
    $currentDt = Parse-HHMM ($now.ToString('HH:mm'))
    # requestsMonth = sum of requests in calendar month containing givenDate
    $requestsMonth = ($dailyRows | Where-Object { $_.Date.Month -eq $givenDate.Month -and $_.Date.Year -eq $givenDate.Year } | Measure-Object -Property Requests -Sum).Sum
    if (-not $requestsMonth) { $requestsMonth = 0 }
    # requestsToday from CSV if present
    $requestsToday = ($dailyRows | Where-Object { $_.Date -eq $givenDate.Date } | Measure-Object -Property Requests -Sum).Sum
    if (-not $requestsToday) { $requestsToday = 0 }
    $mode = '1'   # month-to-date only mode
    $NonInteractive = $true
}
