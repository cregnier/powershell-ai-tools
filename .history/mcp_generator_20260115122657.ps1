...
try {
	...
	# Robust tag parsing: accept array or string, split on commas first, then spaces
	$tags = @()
	if (-not [string]::IsNullOrWhiteSpace($tagsInput)) {
		...
		# if result is single element but contains spaces, split further on whitespace
		if ($tags.Count -eq 1 -and ($tags[0] -match '\s+')) {
			$tags = ($tags[0] -split '\s+') | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' }
		}
	}
	if (-not $tags) { $tags = @() }
	...
	# Final normalization: force array and split a single combined token on comma or whitespace
	$tags = @($tags)
	if ($tags.Count -eq 1) {
		$candidate = $tags[0]
		if ($candidate -like '*,*') { $tags = ($candidate -split ',') | ForEach-Object { $_.Trim() } }
		elseif ($candidate -match '\s+') { $tags = ($candidate -split '\s+') | ForEach-Object { $_.Trim() } }
	}
	# Ensure tags are an array of strings
	$tagsArray = @()
	foreach ($t in $tags) { $tagsArray += [string]$t }
