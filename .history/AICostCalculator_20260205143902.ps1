...
if ($FetchMode) {
    ...
    if ([datetime]::Today -gt $currentCycleEnd -and $mostRecentDate -le $currentCycleEnd) {
        $needsUpdate = $true
    ...
    if ([datetime]::Today.Month -ne $mostRecentDate.Month -or [datetime]::Today.Year -ne $mostRecentDate.Year) {
        $needsUpdate = $true
        $updateReasons += "Currently in $([datetime]::Today.ToString('MMMM yyyy')) but data only through $($mostRecentDate.ToString('MMMM yyyy'))"
    }
