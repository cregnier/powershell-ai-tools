...
.PARAMETER Force
If provided, existing output files will be overwritten and the normalization script will be invoked to regenerate all markdown templates.
...
try {
    ...
    # Prompt for allowed frontmatter keys instead of Author/Tags to avoid introducing unsupported keys
    if (-not $PSBoundParameters.ContainsKey('Model')) {
        $m = Read-Host "Model (default: $Model)"
        if (-not [string]::IsNullOrWhiteSpace($m)) { $Model = $m }
    }
    if (-not $PSBoundParameters.ContainsKey('ArgumentHint')) {
        $ArgumentHint = Read-Host 'Argument hint (optional)'
    }
    if (-not $PSBoundParameters.ContainsKey('Tags')) {
        $tagsInput = Read-Host 'Tools (comma-separated, optional)'
    } else {
        $tagsInput = $Tags
    }
    ...
    function Build-MCPBody {
        ...
        if (-not [string]::IsNullOrWhiteSpace($CustomPrompt)) {
            ...
            $systemHeader += " For each numbered requirement produce the following sections:`n"
            $systemHeader += "- Summary: one-paragraph explanation of the requirement and risk/goal.`n"
            $systemHeader += "- Action Plan: step-by-step concrete actions with exact commands (az/ARM/Bicep/PowerShell) or code snippets.`n"
            $systemHeader += "- Files/Patches: list of files to create or patch; provide unified diffs where applicable.`n"
            $systemHeader += "- Todos: markdown-ready entries for todos.md including file path and commands.`n"
            $systemHeader += "- Tests/Verification: exact commands and checks to validate success.`n`n"
        }

        if ($RawBody -match "^\s*1\)" -or $RawBody -match "^\s*1\.") {
            return ($systemHeader + "Context:" + "`n`n" + $RawBody)
        }
        else {
            return ($systemHeader + "Body:" + "`n`n" + $RawBody)
        }
    }
    ...
    if ($SystemPromptFile) {
        ...
        # Convert any literal "\n" sequences into real newlines for cleanliness
    ...
    if ((Test-Path $outFile) -and -not $Force.IsPresent) {
    ...
    if ($AsMarkdown.IsPresent) {
        ...
        # Build YAML frontmatter using only allowed keys for VS Code templates
        # Supported keys: agent, argument-hint, description, model, name, tools
        $front = @()
        $front += '---'
        $front += "name: $($template.displayName)"
        if (-not [string]::IsNullOrWhiteSpace($template.description)) { $front += "description: $($template.description)" }
        # agent key is required by some processors; use literal 'agent' unless ArgumentHint specifies otherwise
        $front += "agent: agent"
        # include argument-hint and model keys (allowed keys)
        $front += "argument-hint: $ArgumentHint"
        $front += "model: $Model"
        ...
        if ((Test-Path $mdOut) -and -not $Force.IsPresent) {
            throw "Markdown output already exists: $mdOut. Use -Force to overwrite."
        }
        $mdLines -join "`n" | Out-File -FilePath $mdOut -Encoding UTF8
        Write-Host "Wrote markdown template to: $mdOut"

        # If the user requested Force, normalize all markdown outputs across the templates directory
        if ($Force.IsPresent) {
            $normScript = Join-Path $PSScriptRoot 'scripts\normalize_mcp_markdown.ps1'
            if (Test-Path $normScript) {
                & $normScript -Force
            }
            else {
                Write-Host "Normalizer not found: $normScript" -ForegroundColor Yellow
            }
        }
    }

    # Post-generation guidance for VS Code
    # Prefer showing the Markdown path first when AsMarkdown was requested
    if ($AsMarkdown.IsPresent -and (Test-Path $mdOut)) { $primary = $mdOut; $secondary = $outFile }
    else { $primary = $outFile; $secondary = $null }

    Write-Host "`nNext steps for VS Code usage:" -ForegroundColor Cyan
    Write-Host "- Open the file in the workspace: $primary"
    if ($secondary) { Write-Host "- (JSON canonical template also available at: $secondary)" }
